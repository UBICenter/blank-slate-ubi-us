...
if not blank_slate_df_path.exists():
    ...
    blank_slate_df = pd.DataFrame(
        dict(
            baseline_spm_unit_net_income=baseline.calc(
                "spm_unit_net_income", 2022
            ),
            count_child=baseline.calc(
                "is_child", map_to="spm_unit", period=2022
            ),
            count_adult=baseline.calc(
                "is_wa_adult", map_to="spm_unit", period=2022
            ),
            count_senior=baseline.calc(
                "is_senior", map_to="spm_unit", period=2022
            ),
            funded_spm_unit_net_income=funded.calc(
                "spm_unit_net_income", 2022
            ),
...
UBI_FUNDING = (
    (
        blank_slate_df.baseline_spm_unit_net_income
        - blank_slate_df.funded_spm_unit_net_income
    )
